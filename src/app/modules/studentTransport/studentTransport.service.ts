...
const studentTransportAssignment = (payload: TTransportAssignment) => {
  return prisma.$transaction(async (tx) => {
    // check
    const isActiveStudentExist = await tx.studentTransportAssignment.findFirst({
      where: {
    ...
    const transportAssign = tx.studentTransportAssignment.create({
      data: payload,
    });
